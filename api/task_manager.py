"""Async task manager with persisted machine-readable task progress."""
import json
import asyncio
import os
import shutil
...
from . import config
from .progress import ProgressSnapshot, STAGE_LABELS, parse_progress
...
class Task:
    ...
    def to_dict(self) -> dict:
        ...
        d["progress"] = {
            ...
            "stage_label": STAGE_LABELS.get(self.progress_stage, self.progress_stage),
...
class TaskManager:
            ...
            try:
                ...
                for t in data.get("tasks", []):
                    ...
                    if not task.execution_params and task.input_path:
                        try:
                            task.execution_params = json.loads(Path(task.input_path).read_text(encoding="utf-8"))
                        except Exception:
                            pass
    ...
    def create(self, task_type: str, task_input: dict, request_params: Optional[dict] = None) -> Task:
    ...
    def delete(self, task_id: str) -> Task:
        ...
        try:
            out_dir.relative_to(config.OUTPUT_DIR.resolve())
    ...
    def _set_progress(self, task: Task, snap: ProgressSnapshot) -> bool:
        new_values = (
            max(0, min(100, int(snap.percent))), snap.stage, snap.detail,
            int(snap.current_segment), int(snap.total_segments),
        )
        old_values = (
            task.progress_percent, task.progress_stage, task.progress_detail,
            task.current_segment, task.total_segments,
        )
    ...
    async def _run(self, task: Task):
        async with self._gpu_sem:
            ...
            self._set_progress(task, ProgressSnapshot(5, "starting", "正在启动推理进程"))
            self._persist()
            ...
            if not script:
                ...
                self._set_progress(task, ProgressSnapshot(task.progress_percent, "failed", task.error))
                self._persist()
                return
            ...
            if task.task_type in ("avatar_single", "avatar_multi"):
                ...
                if model_type is None:
                    try:
                        model_type = json.loads(Path(task.input_path).read_text(encoding="utf-8")).get("model_type")
                    except Exception:
                        model_type = None
            ...
            env["PYTHONPATH"] = os.pathsep.join([str(config.REPO_ROOT)] + ([existing_pp] if existing_pp else []))
            ...
            try:
                ...
                task.return_code = -1
                task.error = f"failed to launch subprocess: {exc}"

            task.finished_at = time.time()
            if task.return_code == 0:
                task.status = TaskStatus.DONE
                task.outputs = self._collect_outputs(task)
                task.result = {"status": "success", "outputs": task.outputs, "output_count": len(task.outputs)}
                self._set_progress(task, ProgressSnapshot(100, "completed", "视频生成完成", task.total_segments, task.total_segments))
            else:
                task.status = TaskStatus.FAILED
                task.error = self._tail_error(log_path)
                task.result = {"status": "failed", "error": task.error}
                self._set_progress(task, ProgressSnapshot(task.progress_percent, "failed", "请查看运行日志", task.current_segment, task.total_segments))
            self._persist()

    def _build_cmd(self, script: Path, checkpoint: str, task: Task) -> list:
        cmd = [
            "torchrun", f"--nproc_per_node={max(1, config.NUM_GPUS)}", "--nnodes=1",
            "--master_port=29501", str(script), "--task_input", task.input_path,
            "--output_dir", task.output_dir, "--checkpoint_dir", checkpoint,
    ...
    def _collect_outputs(self, task: Task) -> list:
        results = []
        for p in sorted(Path(task.output_dir).glob("*.mp4")):
            stat = p.stat()
            results.append({
                "filename": p.name, "size": stat.st_size, "created_at": stat.st_mtime,
    ...
    def _tail_error(self, log_path: Path, n: int = 40) -> str:
        try:
            ...
            return f"(unreadable log {log_path}: {exc})"
