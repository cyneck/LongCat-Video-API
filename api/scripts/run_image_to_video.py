...
def generate(args):
    ...
    image_path = cfg["cond_image"]
    image = load_image(image_path)
    prompt = cfg["prompt"]
    negative_prompt = cfg.get("negative_prompt", DEFAULT_NEGATIVE_PROMPT)
    resolution = cfg.get("resolution", "480p")  # 480p / 720p
    ...
    if local_rank == 0:
        ...
        write_video(os.path.join(args.output_dir, "output_i2v_refine.mp4"), output_tensor, fps=fps, video_codec="libx264", options={"crf": "10"})
