...
def generate(args):
    ...
    video_path = cfg["cond_video"]
    video = load_video(video_path)
    prompt = cfg["prompt"]
    negative_prompt = cfg.get("negative_prompt", DEFAULT_NEGATIVE_PROMPT)
    num_cond_frames = int(cfg.get("num_cond_frames", 13))
    ...
    if local_rank == 0:
        ...
        output_tensor = torch.from_numpy(np.array(output))
        write_video(os.path.join(args.output_dir, "stage2_distill_480p.mp4"), output_tensor, fps=15, video_codec="libx264", options={"crf": "18"})
    ...
    if local_rank == 0:
        ...
        write_video(os.path.join(args.output_dir, "output_vc_refine.mp4"), output_tensor, fps=fps, video_codec="libx264", options={"crf": "10"})
