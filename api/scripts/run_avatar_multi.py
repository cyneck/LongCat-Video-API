...
def audio_prepare_multi(left_temp_vocal_path, right_temp_vocal_path, generate_duration, left_raw_speech_path, right_raw_speech_path, sample_rate=16000, audio_type="para"):
    ...
    source_duration = len(left_speech_array_ext) / sr
    added_sample_nums = math.ceil((generate_duration - source_duration) * sr)
    if added_sample_nums > 0:
        left_speech_array_ext = np.append(left_speech_array_ext, [0.0] * added_sample_nums)
        right_speech_array_ext = np.append(right_speech_array_ext, [0.0] * added_sample_nums)
...
def generate(args):
    ...
    assert left_raw_speech_path is not None or right_raw_speech_path is not None, "At least one speech is required."
    image_path = input_data["cond_image"]
    audio_type = input_data.get("audio_type", "para")

    resolution = input_data.get("resolution", "480p")
    num_segments = max(1, int(input_data.get("num_segments", 1)))
    ...
    if cp_rank == 0:
        ...
        left_speech_array_ext, right_speech_array_ext, merge_speech = audio_prepare_multi(
        ...
        assert left_full_audio_emb.shape == right_full_audio_emb.shape, "Inconsistent audio embedding shape."
        if use_background_silent_audio:
            assert left_full_audio_emb.shape == back_full_audio_emb.shape, "Inconsistent audio embedding shape between speaker and background."
    ...
    if left_person_bbox is None and right_person_bbox is None:
        ...
        raise NotImplementedError("Not supported bbox type.")
    ...
    if cp_rank == 0:
        ...
        save_video_ffmpeg(output_tensor, os.path.join(output_dir, "segment_1"), merge_speech_path, fps=save_fps, quality=5)
    del output
    torch_gc()
    ...
    # final merged output alias (single-segment case)
